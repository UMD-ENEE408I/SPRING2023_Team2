...
const int udpPort = 3333; //SWITCHES FOR EACH ROBOT/MOUSE
...
void loop() { //aka main method
  ...
  while (true) {//LOOPS FOR INFINITY, where we do everything
    ...
    target_v = outpy[0]; //new theta/angle
  ...
  //----------------- the command to actually move the robot
    ...
    if((pos_left + pos_right)/2 >= target_v){ // if average distance is greater than or = to the set distance
