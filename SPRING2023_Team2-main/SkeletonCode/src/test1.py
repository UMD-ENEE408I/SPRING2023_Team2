...
class Streaming(object):
    def __init__(self):
        ...
        self.K = np.load(output_K)
        D = np.load(output_D)

        balance = 0
        new_K = cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(self.K, D, DIM, np.eye(3), balance=balance)
        self.map1, self.map2 = cv2.fisheye.initUndistortRectifyMap(self.K, D, np.eye(3), new_K, DIM, cv2.CV_16SC2)
    ...
    def cammain(self,vidnum,cordlist,tagid, stop):
        ...
        while True:
            try:
                ret, img = vid.read()
                if first:
                    print('camera {} {}'.format(vidnum, ret))
                    first = False
...
if __name__ == '__main__':
    ...
    #sharks
    thread0 = threading.Thread(target=lambda: cam1.cammain(1, cordlist, tagid0,stop1))
    thread1 = threading.Thread(target=lambda: cam2.cammain(2, cordlist1, tagid1,stop2))
